...
export const getScheduleClosed = createServerAction()
	.input(
		z.object({
	...
	.handler(async ({ input }) => {
		...
		if (!input || Object.keys(input).length === 0) {
			console.log("Usando filtro padrão: canceled, confirmed ou refunded");
			conditions.push(
				sql`${schedule.status} = 'canceled' OR ${schedule.status} = 'confirmed' OR ${schedule.status} = 'refunded'`,
			);
		} else {
